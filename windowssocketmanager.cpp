 ...
 * ----------------------------------------------------------------------------------------------------------------------*/
vector<QString> WindowsSocketManager::resolveHostNameToIP(const char *hostName)
{
   ...
   else
   {
      for (p = hp->h_addr_list; *p != 0; p++)
      {
         ...
         {
            values.push_back(QString::fromUtf8(*q));
         }
      }
   }
   WSACleanup();
...
QString WindowsSocketManager::resolveIPToHostName(const char *ip)
{
   ...
   else
   {
      ...
      hostName       = (hp == NULL) ? "Host information cannot be found." : QString::fromUtf8(hp->h_name, -1);
   }
   WSACleanup();
   return(hostName);
}
